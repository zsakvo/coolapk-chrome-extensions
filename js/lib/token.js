function currentTimeTms() {
  return new Date().getTime();
}

function checkFunDep(value, message) {
  if (typeof value != "function") throw Error(message);
}
...
function coolToken(_base, appId, uuid, time) {
  checkFunDep(digest, "MD5 digest algorithm not found");
  var base =
    _base || "token://com.coolapk.market/c67ef5943784d09750dcfbb31020f0ab?";
...
function xmlhttp(url, asyncb, method) {
  ...
  req.onreadystatechange = function(st) {
    if (req.readyState == XMLHttpRequest.DONE) asyncb(req);
  };
  return req;
}
...
cool = {
  detail: function(id) {
    return "https://api.coolapk.com/v6/feed/detail?id=" + id;
  }
};
